...
async def extract_issue(message: str, available_sites=None):
    ...
    # ── Format sites ──
    if available_sites:
        if isinstance(available_sites, list):
            sites_list    = available_sites
            sites_display = "\n".join(f"- {s}" for s in available_sites)
        else:
            sites_list    = [str(available_sites)]
            sites_display = str(available_sites)
    else:
        sites_list    = []
        sites_display = "No sites available"

    system_prompt = f"""
You are an issue extraction engine.

━━━━━━━━━━━━━━━━━━━━━━
VALID SKILL NAMES (use ONLY one of these — fetched live from DB):
{valid_skills}

Skill mapping rules:
- pipe / water leak / tap / drain / flush / plumber       → match to plumbing skill if present
- wiring / power / switch / socket / light / electric     → match to electrical skill if present
- AC / air conditioning / cooling / heating / ventilation → match to hvac skill if present
- engine / motor / machine / pump / generator             → match to mechanical skill if present
- CCTV / camera / lock / guard / access / alarm           → match to security skill if present
- No match                                                → null

STRICT RULE: NEVER invent a skill name. ONLY return a value from the list above or null.

━━━━━━━━━━━━━━━━━━━━━━
VALID SITE LOCATIONS (use ONLY one of these — fetched live from DB):
{sites_display}

Site matching rules:
- Match the location in the message to the closest site from the list above.
- Use partial/fuzzy match if needed (e.g. "velachery" → "Velachery").
- If no site matches → null.

STRICT RULE: NEVER invent a site. ONLY return a value from the list above or null.
...
{{
  "skill_name": "value from valid skills or null",
  "site_location": "value from valid sites or null",
  "days_to_fix": integer or null,
...
"""
    ...
    print("Extract Issue Response:", response.choices[0].message.content)
    return safe_json_parse(response.choices[0].message.content)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
When routing to create_issue, BEFORE calling the function:

STEP 1 — Scan the user's message for a deadline or number of days.
  Examples of valid days mentions:
  → "fix in 3 days", "2 days", "by friday", "a week", "need it done tomorrow"

STEP 2 — If NO days/deadline is found in the message:
  → DO NOT call create_issue
  → Ask the user: "How many days do you need to fix this issue?"
  → Wait for the user's reply

STEP 3 — User replies with days (e.g. "4", "4 days", "a week"):
  → Combine the original issue message + the days reply
  → NOW call create_issue(message="<original message> (days_to_fix: <N>)")

STRICT RULE:
→ NEVER call create_issue without days_to_fix being confirmed by the user.
→ NEVER assume or guess a default number of days.
→ NEVER use 2, 3, or any number unless the user explicitly said it.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# # ==================================================
# # CLAUDE TOOL FORMAT CONVERTER
# # ==================================================

# def convert_tools_to_claude_format(tools):
#     """Convert OpenAI-style tool definitions to Claude's tool format."""
#     claude_tools = []
#     for tool in tools:
#         fn = tool["function"]
#         claude_tools.append({
...
#         "extend_deadlines":      ["issue_id"],
#         "solver_complete_work":  ["issue_id"],
#         "solver_report_blocker": ["issue_id"],
#         "raise_complaint":       ["issue_id", "message"],
#         "reassign_solver":       ["issue_id", "solver_name"],
#         "query_function":        ["query"],
#         "llm_function":          ["message"],
...
# VALID_SKILLS = ["plumbing", "electrical", "hvac", "mechanical", "security"]
...
# VALID SKILL NAMES (you MUST only use one of these — no other values allowed):
# {VALID_SKILLS}
...
# - Read the user message and map the problem to the closest skill from the list above.
# - ONLY return one of these exact strings: {VALID_SKILLS}
# - Do NOT invent, rephrase, or hallucinate any skill name.
# - Do NOT return values like "pipe repair", "pipe broken", "wiring", "AC", etc.
# - Mapping examples:
#   * pipe / water leak / tap / drain / flush → "plumbing"
#   * wiring / power / switch / socket / light / electric → "electrical"
#   * AC / air conditioning / cooling / heating / ventilation → "hvac"
#   * engine / motor / machine / pump / generator → "mechanical"
#   * CCTV / camera / lock / guard / access / alarm → "security"
# - If the problem does not clearly match any skill → return null
...
#         model="claude-sonnet-4-6",
#         max_tokens=1000,
#         system="Return JSON only. skill_name must be one of: plumbing, electrical, hvac, mechanical, security — or null. Never invent a skill name.",
#         messages=[{"role": "user", "content": prompt}]
#     )
...
#         explanation = claude_client.messages.create(
#             model="claude-sonnet-4-6",
#             max_tokens=4096,
#             system="""
# You are a system explaining database results to users.
...
#         final_answer = explanation.content[0].text
...
#     # ✅ Get last AI message for PENDING tag checks
#     last_ai_msg = next(
#         (m.content for m in reversed(history_messages) if m.type == "ai"),
#         None
#     )

#     # ✅ Check if last AI message was a site clarification
#     if last_ai_msg and "[PENDING:create_issue|site_clarification|" in last_ai_msg:
#         match = re.search(
#             r"\[PENDING:create_issue\|site_clarification\|(.*?)\]",
#             last_ai_msg
...
#     # ✅ Check if last AI message was a days_to_fix clarification for create_issue
#     if last_ai_msg and "[PENDING:create_issue|days_missing|" in last_ai_msg:
#         match = re.search(
#             r"\[PENDING:create_issue\|days_missing\|(.*?)\]",
#             last_ai_msg
#         )
#         if match:
#             original_message = match.group(1)
...
#             if days_match:
#                 days = days_match.group(0)
#                 corrected_message = f"{original_message} (days_to_fix: {days})"
#                 memory.chat_memory.add_user_message(user_input)
#                 memory.chat_memory.add_ai_message(f"Called: create_issue({{'message': '{corrected_message}'}})")
#                 save_memory(session_id, memory)
#                 return {
...
#                 }
#             else:
#                 # User didn't give a number — re-ask and re-tag
#                 clarification = "Please provide the number of days needed to fix this issue."
#                 tagged = f"[PENDING:create_issue|days_missing|{original_message}] {clarification}"
#                 memory.chat_memory.add_user_message(user_input)
#                 memory.chat_memory.add_ai_message(tagged)
#                 save_memory(session_id, memory)
#                 return {"intent": "clarification", "message": clarification}
...
#     # ✅ Build messages list for Claude (no system in messages array)
#     claude_messages = []

#     # Inject Redis history as real message turns
#     for hist_msg in history_messages:
#         if hist_msg.type == "human":
#             claude_messages.append({"role": "user", "content": hist_msg.content})
#         else:
#             claude_messages.append({"role": "assistant", "content": hist_msg.content})
...
#     # ✅ Parse Claude response — find tool_use blocks and text blocks
...
#             # ── INDENT ENFORCEMENT ──
#             if locked_fn and name != locked_fn:
#                 print(f"[Indent Override] Claude chose {name}, forcing {locked_fn}")
#                 name = locked_fn
#                 if locked_fn == "query_function":
#                     args = {"query": user_input}
...
#         # ✅ Special tag for create_issue so days_missing handler catches it next turn
#         if pending_fn == "create_issue":
#             original_msg = user_input
#             tagged = f"[PENDING:create_issue|days_missing|{original_msg}] {clarification}"
#         else:
#             tagged = f"[PENDING:{pending_fn}] {clarification}"
...
#     else:
#         # Claude replied in plain text (clarification or mismatch warning)
#         clarification = text_blocks[0].text if text_blocks else "Could you provide more details?"
#         memory.chat_memory.add_user_message(user_input)
#         memory.chat_memory.add_ai_message(clarification)
#         save_memory(session_id, memory)
#         return {"intent": "clarification", "message": clarification}
