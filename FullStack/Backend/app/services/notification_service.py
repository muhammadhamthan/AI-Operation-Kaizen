...
SYNC-ONLY SERVICE:
  This service runs SMTP I/O which blocks. It must NEVER be called
  from an async context (FastAPI request handler). Instead, the
  async webhook handler should enqueue a Celery task that invokes
  this service with a sync Session.

  See: app/workers/call_tasks._do_escalate_sync → calls this.
  See: app/services/call_service.escalate → now enqueues the email task
       rather than calling this directly.

WAVE A FIXES:
  - Was declared sync but being called from async call_service.escalate
    with an AsyncSession. execute() returned a coroutine that the old
    code never awaited → silent AttributeError swallowed by caller's
    bare except. All escalation emails from the webhook path were
    failing invisibly.
  - Now strictly sync: takes a sync Session only. Async callers must
    dispatch through the Celery task notify_escalation_by_email.
"""
...
class NotificationService:
    """
    SYNC-ONLY. Pass a sync SQLAlchemy Session.
    Do NOT call this from an async FastAPI handler — dispatch a
    Celery task (app.workers.call_tasks.notify_escalation_by_email) instead.
    """
    ...
    def send_escalation_email(self, escalation: Escalation) -> bool:
        ...
        try:
            ...
            if not to_list and not cc_list:
                logger.error(
                    "No recipients for escalation #%s — no managers or supervisors have emails set",
                    escalation.id,
                )
                return False
        ...
        <html><body style="font-family:Arial,sans-serif;max-width:600px;margin:auto;">
          ...
          <div style="border:1px solid #e5e7eb;padding:24px;border-radius:0 0 8px 8px;">
            <table style="width:100%;border-collapse:collapse;">
              <tr><td style="padding:8px;color:#6b7280;width:35%;">Issue</td>
                  <td style="padding:8px;font-weight:bold;">#{issue.id} — {issue.title}</td></tr>
              <tr style="background:#f9fafb;"><td style="padding:8px;color:#6b7280;">Site</td>
                  <td style="padding:8px;">{site_name}</td></tr>
              <tr><td style="padding:8px;color:#6b7280;">Priority</td>
                  <td style="padding:8px;">
                    <span style="background:{color};color:white;padding:2px 10px;border-radius:12px;font-size:12px;">
                      {issue.priority.value.upper()}
                    </span>
                  </td></tr>
              <tr style="background:#f9fafb;"><td style="padding:8px;color:#6b7280;">Assigned Solver</td>
                  <td style="padding:8px;">{solver_name}</td></tr>
              <tr><td style="padding:8px;color:#6b7280;">Missed Calls</td>
                  <td style="padding:8px;color:#dc2626;font-weight:bold;">{missed}</td></tr>
              <tr style="background:#f9fafb;"><td style="padding:8px;color:#6b7280;">Reason</td>
                  <td style="padding:8px;">{escalation.reason}</td></tr>
              <tr><td style="padding:8px;color:#6b7280;">Escalated To</td>
                  <td style="padding:8px;">{escalation.escalated_to_role}</td></tr>
              <tr style="background:#f9fafb;"><td style="padding:8px;color:#6b7280;">Time</td>
                  <td style="padding:8px;">{timestamp}</td></tr>
            </table>
            <div style="margin-top:20px;padding:14px;background:#fef2f2;border-left:4px solid #dc2626;border-radius:4px;">
