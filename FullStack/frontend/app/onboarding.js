/**
 * Kairox AI Opex — Onboarding Flow
 *
 * Drop this file in:  app/onboarding.jsx  (or wherever your initial route is)
 *
 * Then update your Expo Router root (app/index.jsx or _layout.jsx) so that
 * unauthenticated users are redirected here before reaching the login screen.
 *
 * Dependencies already used by the project:
 * react-native, expo-router, @expo/vector-icons, react-native-safe-area-context
 *
...
import { useTheme } from '../src/theme/ThemeContext'; // 📍 IMPORTED THEME
...
// Static particle data (pre-calculated so it never shifts on re-render)
const PARTICLES = [
  ...
  return (
    <Animated.View
      ...
      style={{
        ...
        transform: [
          { translateY: anim.interpolate({ inputRange: [0, 1], outputRange: [0, -18] }) },
        ],
      }}
    />
  );
  ...
  return (
    <Animated.View
      ...
      style={{
        ...
        opacity: 0.18,
        transform: [
          { translateY: pos.interpolate({ inputRange: [0, 1], outputRange: [0, height] }) },
        ],
      }}
    />
  );
...
export default function OnboardingScreen() {
  ...
  const { theme, isDark, toggleTheme } = useTheme(); // 📍 DESTRUCTURED THEME
  ...
  return (
    <View style={[styles.root, { backgroundColor: screenBg }]}>
      ...
      <View style={StyleSheet.absoluteFill} pointerEvents="none">
        {Array.from({ length: 9 }).map((_, i) => (
          <View
            ...
            style={[
              styles.gridLine,
              { top: `${(i + 1) * 10}%`, width: '100%', height: 1, backgroundColor: gridLineColor },
            ]}
          />
        ))}
        {Array.from({ length: 7 }).map((_, i) => (
          <View
            ...
            style={[
              styles.gridLine,
              { left: `${(i + 1) * 14}%`, width: 1, height: '100%', backgroundColor: gridLineColor },
            ]}
          />
        ))}
        {/* Corner brackets */}
      ...
      {/* ── Particles ── */}
      {PARTICLES.map((p, i) => (
        <Particle key={i} {...p} accent={slide.accent} />
      ))}
      ...
      <SafeAreaView edges={['top', 'bottom']} style={styles.safeArea}>
        ...
        >
          ...
          {/* Right Actions: Theme Toggle + Skip */}
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
            ...
            <Animated.View style={{ opacity: skipFade }}>
              <TouchableOpacity
                onPress={handleSkip}
                style={[styles.skipBtn, { backgroundColor: skipBg, borderColor: skipBorder }]}
                activeOpacity={0.7}
                hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}
              >
          ...
          >
            ...
            <Animated.View
              ...
              style={[
                styles.iconHalo,
                {
                  width: iconBoxSize + 34,
                  height: iconBoxSize + 34,
                  borderRadius: (iconBoxSize + 34) / 2,
                  backgroundColor: slide.glowColor,
                  opacity: glowOpac,
                  transform: [{ scale: glowSc }],
                },
            ...
            {[0, 90, 180, 270].map((deg) => {
              const r = outerRing / 2 - 6;
              const rad = (deg * Math.PI) / 180;
              const cx = outerRing / 2;
              return (
                <View
                  key={deg}
                  style={{
                    ...
                    left: cx + r * Math.cos(rad) - 3.5,
                    top: cx + r * Math.sin(rad) - 3.5,
                    opacity: 0.9,
                  }}
                />
              );
          ...
          <View style={[styles.textSection, isWide && styles.textSectionWide]}>
            ...
            <Animated.Text
              style={[
                ...
                { color: slide.accent },
                isWide && styles.slideSubtitleWide,
                {
                  opacity: contentFade,
                  transform: [{ translateY: contentSlide }],
                },
            ...
            <Animated.View
              style={[
                styles.divider,
                { backgroundColor: slide.accent, opacity: contentFade },
              ]}
            />
        ...
        >
              ...
              >
                <Animated.View
                  style={[
                    styles.dot,
                    {
                      backgroundColor: i === currentIndex ? slide.accent : dotInactive,
                      width: dotWidths[i],
                    },
                  ]}
                />
          ...
          <View style={styles.navRow}>
            <Text style={styles.counter}>
              <Text style={{ color: slide.accent, fontWeight: '700' }}>
                {String(currentIndex + 1).padStart(2, '0')}
              </Text>
              <Text style={{ color: textFaint }}>
                {' / '}
