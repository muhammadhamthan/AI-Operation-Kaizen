...
const GROUPS = [
  ...
  {
    id: 'projectflow',
    title: 'ProjectFlow Intelligence (per-site)',
    items: [
      'Tables: site_contracts, site_tasks (with depends_on_task_id)',
      'Tables: site_materials (id, site_id, name, qty, unit, vendor_id, expected_arrival, actual_arrival, status, linked_stage_id, recalibration_triggered)',
      'Tables: site_vendors (id, site_id, name, contact, status, dispatch_eta, last_contact_at, delay_days)',
      'Table: vendor_followups (id, vendor_id, channel, sent_at, response_at)',
      'Table: site_flow_alerts (id, site_id, severity, kind, text, at, resolved)',
      'Table: site_flow_recalibrations (id, site_id, triggered_by, reason, affected_task_ids[], at)',
      'GET    /api/v1/sites/:id/projectflow            (whole bundle: timeline + materials + vendors + weather + alerts + health_score)',
      'GET / POST / PATCH / DELETE /api/v1/sites/:id/timeline|tasks   (cascade on PATCH end_date)',
      'GET / POST / PATCH /api/v1/sites/:id/materials',
      'GET / POST / PATCH /api/v1/sites/:id/vendors',
      'POST   /api/v1/sites/:id/vendors/:vid/follow-up  (logs + triggers WhatsApp template)',
      'POST   /api/v1/sites/:id/projectflow/recalibrate (manual MD trigger; logs to recalibrations table)',
      'GET    /api/v1/sites/:id/weather/forecast        (proxy to weather API; cached 1h)',
      'POST   /api/v1/sites/:id/projectflow/alerts/:aid/dismiss',
      'AI alert generator: fires "material_dependency_mismatch", "vendor_delay_risk", "timeline_drift_corrected", "execution_continuity_restored", "environmental" alerts when underlying conditions change',
      'Health score formula: aligned_count / total_count where aligned = stages(completed|in_progress) ∪ materials(arrived|on_time) ∪ vendors(confirmed)',
      'Server-side cascade on stage PATCH: shift dependents + extend contract end + auto-insert "timeline_drift_corrected" alert',
    ],
  },
];
